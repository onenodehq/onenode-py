...
class Image:
    ...
    def __init__(
        self,
        data: Optional[Union[str, bytes, bytearray, io.IOBase, "np.ndarray", "PILImage.Image"]] = None,  # Various image data formats
        ...
        # Handle different input types
            ...
            if data.startswith("data:"):
                ...
                self.mime_type = mime_type or url_mime_type
            else:
            ...
            try:
                ...
                buffer = io.BytesIO()
                format_type = mime_type.split('/')[-1].upper() if '/' in mime_type else 'PNG'
                if format_type == 'JPG': format_type = 'JPEG'
            ...
            try:
                buffer = io.BytesIO()
                format_type = mime_type.split('/')[-1].upper() if '/' in mime_type else data.format or 'PNG'
                if format_type == 'JPG': format_type = 'JPEG'
                data.save(buffer, format=format_type)
                self.data = buffer.getvalue()
                self.mime_type = mime_type or f'image/{data.format.lower()}' if data.format else 'image/png'
            except Exception as e:
                raise ValueError(f"Failed to convert PIL Image: {e}")
        else:
            raise ValueError("Invalid data type: must be string (base64/data URL), bytes, bytearray, file-like object, numpy array, or PIL Image")
    ...
    def _deserialize(cls, data: Dict[str, Any]) -> "Image":
        ...
        instance = cls(data=data_value, mime_type=mime_type)
